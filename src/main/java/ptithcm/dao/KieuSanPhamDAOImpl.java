...
public class KieuSanPhamDAOImpl implements KieuSanPhamDAO{
	...
	@Override
	public boolean kiemTraTenKieuTonTai(String tenKieu) {
	    Session session = sessionFactory.getCurrentSession();
	    String hql = "SELECT COUNT(k) FROM KieuSanPhamEntity k WHERE k.tenKieu = :tenKieu";
	    Query query = session.createQuery(hql);
	    query.setParameter("tenKieu", tenKieu);
